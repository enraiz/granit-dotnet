...
{
    private TestQueryingDbContextFactory _factory = null!;
    private EfCoreSavedViewStore _store = null!;

    public ValueTask InitializeAsync()
    {
        _factory = new TestQueryingDbContextFactory();
    ...
    /// <summary>
    /// In-memory factory for <see cref="QueryingDbContext"/> used by tests.
    /// </summary>
    private sealed class TestQueryingDbContextFactory : IDbContextFactory<QueryingDbContext>, IDisposable
    {
        private readonly DbContextOptions<QueryingDbContext> _options =
            new DbContextOptionsBuilder<QueryingDbContext>()
                .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
                .Options;

        public QueryingDbContext CreateDbContext() => new(_options);

        public Task<QueryingDbContext> CreateDbContextAsync(CancellationToken cancellationToken = default) =>
            Task.FromResult(new QueryingDbContext(_options));
