...
public sealed class SnsMobilePushSenderTests
{
    [Fact]
    public void Class_Implements_IMobilePushSender()
    {
        (SnsMobilePushSender sut, _, _) = CreateSender();
        sut.ShouldBeAssignableTo<IMobilePushSender>();
    }

    [Fact]
    public void Class_IsInternal() =>
        typeof(SnsMobilePushSender).IsNotPublic.ShouldBeTrue();

    [Fact]
    public void Class_IsSealed() =>
        typeof(SnsMobilePushSender).IsSealed.ShouldBeTrue();
    ...
    {
        (SnsMobilePushSender sut, ISnsMobilePushTransport transport, _) = CreateSender();
    ...
    {
        (SnsMobilePushSender sut, ISnsMobilePushTransport transport, IMobilePushEventPublisher publisher) = CreateSender();
    ...
    private static (SnsMobilePushSender Sut, ISnsMobilePushTransport Transport, IMobilePushEventPublisher Publisher) CreateSender()
    {
        ISnsMobilePushTransport transport = Substitute.For<ISnsMobilePushTransport>();
        IMobilePushEventPublisher publisher = Substitute.For<IMobilePushEventPublisher>();

        SnsMobilePushOptions options = new()
        {
            Region = "eu-west-1",
            PlatformApplicationArn = "arn:aws:sns:eu-west-1:123456789:app/GCM/MyApp",
        };

        SnsMobilePushSender sut = new(
            Microsoft.Extensions.Options.Options.Create(options),
            NullLogger<SnsMobilePushSender>.Instance,
            transport,
            publisher);
