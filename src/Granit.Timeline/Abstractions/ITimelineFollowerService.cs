...
public interface ITimelineFollowerService
{
    ...
    Task FollowAsync(string userId, string entityType, string entityId, CancellationToken ct = default);

    /// <summary>Unsubscribes a user from an entity.</summary>
    Task UnfollowAsync(string userId, string entityType, string entityId, CancellationToken ct = default);

    /// <summary>Returns the user IDs of all followers of an entity.</summary>
    Task<IReadOnlyList<string>> GetFollowerIdsAsync(string entityType, string entityId, CancellationToken ct = default);

    /// <summary>Returns whether a user is following an entity.</summary>
    Task<bool> IsFollowingAsync(string userId, string entityType, string entityId, CancellationToken ct = default);
}
