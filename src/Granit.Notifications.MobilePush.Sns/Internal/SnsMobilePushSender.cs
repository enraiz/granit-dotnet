...
/// </remarks>
internal sealed partial class SnsMobilePushSender(
    IOptions<SnsMobilePushOptions> options,
    ILogger<SnsMobilePushSender> logger,
    ISnsMobilePushTransport transport,
    IMobilePushEventPublisher eventPublisher) : IMobilePushSender
{
    ...
    public async Task SendAsync(MobilePushMessage message, CancellationToken cancellationToken = default)
    {
        ...
        using Activity? activity = NotificationsMobilePushSnsActivitySource.Source.StartActivity(
            NotificationsMobilePushSnsActivitySource.Operations.Send);
        activity?.SetTag(NotificationsMobilePushSnsActivitySource.Tags.DeviceCount, message.DeviceTokens.Count);
