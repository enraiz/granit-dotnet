...
public interface ILegalAgreementChecker
{
    ...
    /// </summary>
    Task<bool> HasAcceptedLatestAsync(Guid userId, string documentId, CancellationToken ct = default);

    /// <summary>
    /// Returns the full consent history for a user, ordered by date (most recent first).
    /// </summary>
    Task<IReadOnlyList<LegalAgreementBase>> GetUserAgreementsAsync(Guid userId, CancellationToken ct = default);
}
